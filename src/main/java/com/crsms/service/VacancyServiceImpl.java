...
public class VacancyServiceImpl implements VacancyService {
  ...
  private static Logger logger = LogManager.getLogger(VacancyServiceImpl.class);

  @Override
  public List<VacancyJsonDto> getAllVacancies() {
    ...
    if (links.isEmpty()) logger.error("Links not found");
    Long id = 1L;
    for (Element el : links) {
      VacancyJsonDto vacation = new VacancyJsonDto();
      vacation.setId(id);
      String name = el.getElementsByTag("h3").text();
      if (name.equals("")) logger.error("Headers not found");
